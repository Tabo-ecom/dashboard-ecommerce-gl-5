"""
Ecommerce Profit Dashboard v6.2
Restored Premium Visuals + Logic Fixes
(Multi-upload fixed, Cyclic Mapping, Auto-Testeo)
"""
...
# ⚙️ CONFIGURATION
# -----------------------------------------------------------------------------
st.set_page_config(page_title="T-PILOT v6.2", page_icon="✈️", layout="wide", initial_sidebar_state="expanded")
...
# -----------------------------------------------------------------------------
# 🎨 PREMIUM CSS (VISUALES RESTAURADOS)
# -----------------------------------------------------------------------------
st.markdown("""<style>
...
/* Cards Premium */
.kcard { background: linear-gradient(180deg, #131A2B, #0F1420); border: 1px solid #1E293B; border-radius: 16px; padding: 1.5rem; position: relative; transition: all 0.2s; overflow: hidden; margin-bottom: 1rem; }
.kcard:hover { border-color: rgba(16,185,129,0.4); box-shadow: 0 4px 20px rgba(0,0,0,0.3); } 
.kcard.green { border-color: rgba(16,185,129,0.3); } .kcard.red { border-color: rgba(239,68,68,0.3); }
...
.icon.g { background: rgba(16,185,129,0.15); color: #10B981; } .icon.r { background: rgba(239,68,68,0.15); color: #EF4444; } .icon.b { background: rgba(59,130,246,0.15); color: #3B82F6; } .icon.w { background: rgba(148,163,184,0.15); color: #94A3B8; } .icon.p { background: rgba(139,92,246,0.15); color: #8B5CF6; }
...
.val.green { color: #10B981; } .val.red { color: #EF4444; } .val.white { color: #F1F5F9; } .val.blue { color: #3B82F6; } .val.purple { color: #8B5CF6; }
...
def process_data(df, start, end, country_name, g_ads_local, campaign_map):
    ...
    return {"orders":df_ord,"kpis":{"n_tot":n_tot,"n_ent":n_ent,"n_can":n_can,"n_dev":n_dev,"n_tra":n_tra,"ing_ent":ie,"costo_prod":cpe,"flete_ent":fe,"flete_dev":fd,"flete_tra":ft,"ads":g_ads_local,"utilidad":ur},"prod_summary":pd.DataFrame(psum)}
...
with st.sidebar:
    ...
    with st.expander("🔑 Credenciales API", expanded=False):
        ...
        if st.button("Guardar"): save_cfg("fb_tok", fb_tok); save_cfg("oa_key", oa_key); st.rerun()
...
# -----------------------------------------------------------------------------
# 🚀 LANDING PAGE (MULTI-UPLOAD FIX)
# -----------------------------------------------------------------------------
if not st.session_state.files_data:
    st.title("✈️ T-PILOT v6.2")
...
# ADS & MAPPING (FIXED: SELECTBOX + UNLINK LOGIC)
df_ads=pd.DataFrame()
...
if not df_ads.empty and uploaded_data:
    with st.expander("🔗 Mapeo de Campañas (Cíclico)", expanded=False):
        ...
        if st.button("🪄 Auto-Mapear (IA)"):
            if not oa_key: st.error("Falta Key IA"); 
            else: 
                nm=ia_auto_map(oa_key, unmapped, list(all_p))
                if nm: cm.update(nm); save_json(CAMPAIGN_MAP_FILE,cm); st.rerun()
...
st.markdown("<br><center style='color:#64748B;font-size:0.8rem'>T-PILOT v6.2 · Ultimate Premium</center>", unsafe_allow_html=True)
