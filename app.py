...
Currency Conversion · AI Campaign Mapping · Product Filters
"""
...
def get_trm():
    ...
    try:
        ...
        if r.ok:
            d=r.json(); rates["COP_USD"]=d["rates"].get("COP",4200); rates["COP_GTQ"]=rates["COP_USD"]/d["rates"].get("GTQ",7.75)
    except: pass
    return rates
...
def ai_map(camps,prods,key):
    if not key: return {}
    prompt=f"Empareja campañas FB con productos Dropi.\nCampañas:{json.dumps(camps)}\nProductos:{json.dumps(prods)}\nResponde SOLO JSON: {{\"campaña\":\"producto\"}}"
    try:
        r=requests.post("https://api.openai.com/v1/chat/completions",headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"},json={"model":"gpt-4o-mini","messages":[{"role":"user","content":prompt}],"temperature":0.1,"max_tokens":2000},timeout=30)
        txt=r.json()["choices"][0]["message"]["content"].strip()
        txt=re.sub(r'^```json\s*','',txt); txt=re.sub(r'```$','',txt)
        return json.loads(txt)
    except: return {}

def render_logistics(label, k, color_map, pn=None):
    """Render logistics grid for given kpis dict."""
    ...
    <div class="lgrid"><div class="litem" style="border-color:rgba(239,68,68,0.3)"><p>❌ CANCELADO</p><h3 style="color:{C['loss']}">{nc:,}</h3><span class="badge" style="background:rgba(239,68,68,0.15);color:#EF4444">{(nc/no*100):.0f}% total</span></div></div>
    <div class="lgrid"><div class="litem" style="border-color:rgba(16,185,129,0.3)"><p>✅ ENTREGADO</p><h3 style="color:{C['profit']}">{ne:,}</h3><span class="badge" style="background:rgba(16,185,129,0.15);color:#10B981">{p(ne)}</span></div><div class="litem" style="border-color:rgba(59,130,246,0.3)"><p>🚚 TRÁNSITO</p><h3 style="color:{C['blue']}">{nt:,}</h3><span class="badge" style="background:rgba(59,130,246,0.15);color:#3B82F6">{p(nt)}</span></div><div class="litem" style="border-color:rgba(245,158,11,0.3)"><p>↩ DEVOLUCIÓN</p><h3 style="color:{C['warn']}">{nv:,}</h3><span class="badge" style="background:rgba(245,158,11,0.15);color:#F59E0B">{p(nv)}</span></div><div class="litem" style="border-color:rgba(249,115,22,0.3)"><p>⚠ NOVEDAD</p><h3 style="color:{C['orange']}">{nn:,}</h3><span class="badge" style="background:rgba(249,115,22,0.15);color:#F97316">{p(nn)}</span></div><div class="litem" style="border-color:rgba(100,116,139,0.3)"><p>⏳ OTROS</p><h3 style="color:{C['muted']}">{nother:,}</h3><span class="badge" style="background:rgba(100,116,139,0.15);color:#94A3B8">{p(nother)}</span></div></div>"""

def render_download_logistics(df_ord, F, label_status, pats, key_prefix):
    """Download button for filtered orders by status."""
    if not F.get("ESTATUS"): return
    filtered = df_ord[df_ord["ESTATUS"].apply(lambda s: ms(s, pats))]
    if not filtered.empty:
        st.download_button(f"📥 Descargar {label_status} ({len(filtered)})", to_excel(filtered),
                          file_name=f"{label_status.lower().replace(' ','_')}.xlsx",
...
cfg=load_cfg()
with st.sidebar:
    st.markdown("### 📊 Profit Dashboard v5.0")
    st.divider()
    ...
    if usar_api:
        ...
        if accts:
            opts={f"{a['name']} ({a['id']})":a["id"] for a in accts}
            ok=list(opts.keys())
            sel_all=st.checkbox("✅ Seleccionar TODAS las cuentas",key="sel_all_fb")
            if sel_all: fb_cids=[opts[k] for k in ok]; st.caption(f"✅ {len(fb_cids)} cuentas")
            else:
                sel=st.multiselect("Cuentas FB",ok,key="fb_ms")
            ...
            if fb_token and not accts: st.caption("⚠ No se encontraron cuentas. Verifica el token.")
    ...
    ads_manual={}
    if not usar_api:
        for pn in PAISES: ads_manual[pn]=st.number_input(f"Ads {pn}",min_value=0.0,value=0.0,step=1000.0,key=f"gm_{pn}")
    st.divider()
...
# ═══ FIX #1: MULTI-FILE UPLOAD + CLEAR ═══
if not any_data:
    st.divider()
    st.markdown('<p class="section-hdr">Sube tus archivos</p>',unsafe_allow_html=True)
    st.markdown("Puedes subir **varios archivos a la vez** — se auto-detecta el país de cada uno.")
    files=st.file_uploader("Arrastra tus archivos aquí",type=["csv","xlsx","xls"],accept_multiple_files=True,key="up_multi")
    if files:
        for f in files:
            ...
            else: st.warning(f"⚠ No se detectó país para {f.name}")
...
for pn in PAISES:
    ...
    if "PRODUCTO" in df_f.columns:
        ...
        else: ag=build_groups(sorted(df_f["PRODUCTO"].dropna().unique())); df_f=apply_groups(df_f,ag); PM[pn]=ag; save_mappings(PM)
    df_ord,F=agg_orders(df_f)
    if usar_api:
        r=crl.get(pn,0)/max(tr,1); gfc=cop_to(gfb_cop*r,mon,trm); gtc=cop_to(gtt_cop*r,mon,trm); gfcc=gfb_cop*r; gtcc=gtt_cop*r
    else: gfc=ads_manual.get(pn,0); gtc=0; gfcc=to_cop(gfc,mon,trm); gtcc=0
    kpis=calc_kpis(df_ord,F,gfc,gtc); kpis["g_fb_cop"]=gfcc; kpis["g_tt_cop"]=gtcc; kpis["g_ads_cop"]=gfcc+gtcc
    CD[pn]={"df":df_f,"df_ord":df_ord,"kpis":kpis,"F":F}
if not CD: st.warning("Sin datos."); st.stop()

# Product groups and campaign mapping
with st.expander("📦 Agrupación de Productos",expanded=False):
    for pn,cd in CD.items():
        ...
        for gn,ms_ in sv.items():
            for m in ms_: rows.append({"Original":m,"Grupo":gn})
        if not rows:
            for p in sorted(cd["df"]["PRODUCTO"].dropna().unique()): rows.append({"Original":p,"Grupo":extraer_base(p)})
...
if not all_camps.empty:
    with st.expander("🔗 Mapeo Campañas → Productos",expanded=False):
        ...
        for pn2,cd2 in CD.items():
            if "GRUPO_PRODUCTO" in cd2["df_ord"].columns: gl.extend(cd2["df_ord"]["GRUPO_PRODUCTO"].dropna().unique().tolist())
        gl=sorted(set(gl)); cn=sorted(all_camps["campaign_name"].unique().tolist()); po=["-- Sin asignar --"]+gl
        ac1,ac2=st.columns([1,2])
        with ac1:
            if st.button("🪄 Auto-Mapear IA",key="ai_m"):
                if oai_key:
                    with st.spinner("IA..."): res=ai_map(cn,gl,oai_key)
                    if res: cm_saved.update(res); save_camp_map(cm_saved); st.rerun()
                else: st.warning("Ingresa OpenAI Key")
        with ac2: st.caption(f"{len(cn)} campañas · {sum(1 for c in cn if cm_saved.get(c,'')!='-- Sin asignar --' and cm_saved.get(c,''))} enlazadas")
        for camp in cn:
            cur=cm_saved.get(camp,"-- Sin asignar --"); idx=po.index(cur) if cur in po else 0
            lbl="✅" if idx>0 else "⚪"
            nv=st.selectbox(f"{lbl} {camp}",po,index=idx,key=f"cm_{camp}")
            if nv!=cur: cm_saved[camp]=nv; save_camp_map(cm_saved)
    for camp in cn:
        prod=cm_saved.get(camp,"")
        if prod and prod!="-- Sin asignar --":
            sp=all_camps[all_camps["campaign_name"]==camp]["spend"].sum()
            gxp[prod.upper()]=gxp.get(prod.upper(),0)+sp
...
with tabs[0]:
    ...
    # FIX #9: Global facturación + orders chart
    all_daily=[]
    for pn,cd in CD.items():
        if "FECHA" in cd["df_ord"].columns:
            d=cd["df_ord"].groupby(cd["df_ord"]["FECHA"].dt.date).agg(fac=("TOTAL DE LA ORDEN","sum"),ords=("ID","count")).reset_index()
            d.columns=["Fecha","Fac","Ords"]; d["Fac"]=d["Fac"].apply(lambda v:to_cop(v,PAISES[pn]["moneda"],trm))
            all_daily.append(d)
    if all_daily:
        dg=pd.concat(all_daily).groupby("Fecha").sum().reset_index()
        fig=go.Figure()
        fig.add_trace(go.Bar(x=dg["Fecha"],y=dg["Ords"],name="Órdenes",marker_color=C["blue"],opacity=.6,yaxis="y2",hovertemplate="%{y}<extra></extra>"))
        fig.add_trace(go.Scatter(x=dg["Fecha"],y=dg["Fac"],name="Facturación",line=dict(color=C["profit"],width=2.5),fill="tozeroy",fillcolor="rgba(16,185,129,0.08)",hovertemplate="%{y:,.0f}<extra></extra>"))
        fig.update_layout(**pl(title="FACTURACIÓN Y ÓRDENES GLOBALES (COP)",yaxis2=dict(overlaying="y",side="right",gridcolor="rgba(0,0,0,0)",title="Órdenes")))
        st.plotly_chart(fig,use_container_width=True,key="gl_fac")

    st.divider()
    for pn,cd in CD.items():
        k=cd["kpis"];pi=PAISES[pn];uc="green" if k["u_real"]>=0 else "red"
        cn=f'<span style="font-size:.65rem;color:#475569;font-family:JetBrains Mono">({fmt_cop(k.get("g_ads_cop",0))} COP)</span>' if pi["moneda"]!="COP" else ""
        st.markdown(f'<div class="row2 r4" style="margin-bottom:.5rem"><div class="kcard"><div class="icon w">📦</div><div class="lbl">{pi["flag"]} {pn.upper()}</div><div class="val lg white">{k["n_ord"]:,}</div><div class="sub">{k["n_ent"]} ent · {k["tasa_ent"]:.0f}%</div></div><div class="kcard green"><div class="icon g">📈</div><div class="lbl">FACTURADO</div><div class="val lg green">{fmt(k["fact_neto"],pn)}</div></div><div class="kcard red"><div class="icon r">🎯</div><div class="lbl">ADS</div><div class="val md red">{fmt(k["g_ads"],pn)}</div>{cn}</div><div class="kcard {uc}"><div class="icon {"g" if k["u_real"]>=0 else "r"}">💰</div><div class="lbl">UTILIDAD</div><div class="val lg {uc}">{fmt(k["u_real"],pn)}</div></div></div>',unsafe_allow_html=True)

    # FIX #7: Logistics per country + global
    st.divider()
    # Global
    gk={f:sum(cd["kpis"][f] for cd in CD.values()) for f in ["n_ord","n_ent","n_can","n_tra","n_dev","n_nov","n_otr"]}
    ...
    for pn,cd in CD.items():
        ...
        # FIX #8: Download buttons
        dc1,dc2,dc3,dc4=st.columns(4)
        with dc1: render_download_logistics(cd["df_ord"],cd["F"],"Entregados",STATUS_ENT,f"dl_ent_{pn}")
        with dc2: render_download_logistics(cd["df_ord"],cd["F"],"Cancelados",STATUS_CAN,f"dl_can_{pn}")
        with dc3: render_download_logistics(cd["df_ord"],cd["F"],"Tránsito",STATUS_TRA,f"dl_tra_{pn}")
        with dc4: render_download_logistics(cd["df_ord"],cd["F"],"Devolución",STATUS_DEV,f"dl_dev_{pn}")


# ═══ TAB: PROYECCIÓN GLOBAL (FIX #10) ═══
with tabs[1]:
    st.markdown('<p class="section-hdr">Proyección Global (todos los países en COP)</p>',unsafe_allow_html=True)
    gdg=st.slider("% Entrega Global",50,100,80,key="gdg")
    grb=st.number_input("Colchón Dev.",value=1.40,min_value=1.0,max_value=3.0,step=0.05,key="grb")
    gp_rows=[]
    for pn,cd in CD.items():
        k=cd["kpis"];gc=gcol_of(cd)
        if not gc: continue
        df_f=cd["df"];df_ord=cd["df_ord"];F=cd["F"];mon=PAISES[pn]["moneda"]
        ...
        for prod in df_f[gc].dropna().unique():
            ...
            oe=od*gdg/100;odv=od-oe;ue=uds*gdg/100;cp=ue*pp;fl=oe*fp+odv*fp*grb;ap=gxp.get(prod.upper(),0);ip=oe*ixo;ut=ip-cp-fl-ap
            gp_rows.append({"País":f"{PAISES[pn]['flag']} {pn}","Producto":prod,"Órdenes":int(od),"Ingreso_COP":to_cop(ip,mon,trm),"Costo_COP":to_cop(cp,mon,trm),"Fletes_COP":to_cop(fl,mon,trm),"Ads_COP":to_cop(ap,mon,trm),"Utilidad_COP":to_cop(ut,mon,trm)})
    if gp_rows:
        dfgp=pd.DataFrame(gp_rows)
        ti=dfgp["Ingreso_COP"].sum();tu=dfgp["Utilidad_COP"].sum();to2=dfgp["Órdenes"].sum()
        st.markdown(f'<div class="row2 r3"><div class="kcard"><div class="icon w">📦</div><div class="lbl">ÓRDENES</div><div class="val lg white">{to2:,}</div></div><div class="kcard green"><div class="icon g">📊</div><div class="lbl">INGRESO PROY.</div><div class="val lg green">{fmt_cop(ti)}</div></div><div class="kcard {"green" if tu>=0 else "red"}"><div class="icon {"g" if tu>=0 else "r"}">📈</div><div class="lbl">UTILIDAD PROY.</div><div class="val lg {"green" if tu>=0 else "red"}">{fmt_cop(tu)}</div></div></div>',unsafe_allow_html=True)
        # By country
        by_c=dfgp.groupby("País").agg({"Ingreso_COP":"sum","Utilidad_COP":"sum","Órdenes":"sum"}).reset_index()
        mx=max(by_c["Ingreso_COP"].max(),1);bh=""
        for _,r in by_c.iterrows():
            bp=min(r["Ingreso_COP"]/mx*100,100);utc=C["profit"] if r["Utilidad_COP"]>=0 else C["loss"]
            bh+=f'<div style="display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #111827"><div style="width:160px;font-size:.85rem;color:{C["text"]}">{r["País"]}</div><div style="flex:1;margin:0 12px"><div style="background:#111827;border-radius:3px;height:20px;overflow:hidden"><div style="width:{bp:.0f}%;height:100%;background:{C["profit"]};opacity:.4"></div></div></div><div style="width:120px;text-align:right;font-family:JetBrains Mono;font-size:.82rem;color:{C["text"]}">{fmt_cop(r["Ingreso_COP"])}</div><div style="width:120px;text-align:right;font-family:JetBrains Mono;font-size:.82rem;color:{utc}">{fmt_cop(r["Utilidad_COP"])}</div><div style="width:60px;text-align:right;font-size:.75rem;color:{C["sub"]}">{int(r["Órdenes"])}</div></div>'
...
for idx,(pn,cd) in enumerate(CD.items()):
    with tabs[idx+2]:
        k=cd["kpis"];df_ord=cd["df_ord"];df_f=cd["df"];F=cd["F"];pi=PAISES[pn];gc=gcol_of(cd)
        prods=sorted(df_f[gc].dropna().unique()) if gc else []
        ct1,ct2,ct3,ct4=st.tabs(["🌡 Termómetro","📊 Proyecciones","💰 Operación","📋 Órdenes"])

        with ct1:
            at=f'<span style="font-size:.65rem;color:#475569">({fmt_cop(k.get("g_ads_cop",0))} COP)</span>' if pi["moneda"]!="COP" else ""
            ...
            # FIX #6: Product filter for charts
            if gc and F.get("FECHA"):
                ...
                # FIX #9: dual axis facturación + orders
                daily=df_ch.groupby(df_ch["FECHA"].dt.date).agg(fac=("TOTAL DE LA ORDEN","sum"),ords=("ID","count")).reset_index()
                daily.columns=["Fecha","Fac","Ords"]
                gc1,gc2=st.columns(2)
                with gc1:
                    fig=go.Figure()
                    fig.add_trace(go.Bar(x=daily["Fecha"],y=daily["Ords"],name="Órdenes",marker_color=C["blue"],opacity=.5,yaxis="y2",hovertemplate="%{y}<extra></extra>"))
                    fig.add_trace(go.Scatter(x=daily["Fecha"],y=daily["Fac"],name="Facturación",line=dict(color=C["profit"],width=2),fill="tozeroy",fillcolor="rgba(16,185,129,0.1)",hovertemplate="%{y:,.0f}<extra></extra>"))
                    fig.update_layout(**pl(title="FACTURACIÓN + ÓRDENES",yaxis2=dict(overlaying="y",side="right",gridcolor="rgba(0,0,0,0)")))
                    st.plotly_chart(fig,use_container_width=True,key=f"fac_{pn}")
                with gc2:
                    if F.get("ESTATUS"):
                        edf=df_ch["ESTATUS"].value_counts().reset_index();edf.columns=["E","N"]
                        cm={s:(C["profit"] if "ENTREGADO" in s else C["loss"] if "CANCELADO" in s else C["orange"] if "DEVOLUCION" in s else C["blue"] if ms(s,STATUS_TRA) else C["muted"]) for s in edf["E"]}
                        f3=px.pie(edf,names="E",values="N",hole=.55,color="E",color_discrete_map=cm)
                        f3.update_layout(**pl(showlegend=True,title="ESTADOS"));f3.update_traces(textinfo="percent")
            ...
            dc1,dc2,dc3,dc4=st.columns(4)
            with dc1: render_download_logistics(df_ord,F,"Entregados",STATUS_ENT,f"dl_ent2_{pn}")
            with dc2: render_download_logistics(df_ord,F,"Cancelados",STATUS_CAN,f"dl_can2_{pn}")
            with dc3: render_download_logistics(df_ord,F,"Tránsito",STATUS_TRA,f"dl_tra2_{pn}")
            with dc4: render_download_logistics(df_ord,F,"Devolución",STATUS_DEV,f"dl_dev2_{pn}")

            # FIX #11: Product summary with logistics % and ads
            if gc:
                ...
                for prod in prods:
                    ...
                    nnc_=np_-nc_; ap_=gxp.get(prod.upper(),0)
                    pr.append({"Producto":prod,"Pedidos":np_,"Facturado":fp_,"Ent":ne_,"Can":nc_,"Tra":nt_,"Dev":nv_,"Ads":ap_,
                              "%Ent":f"{(ne_/nnc_*100):.0f}%" if nnc_ else "-","%Can":f"{(nc_/np_*100):.0f}%" if np_ else "-",
                              "%Tra":f"{(nt_/nnc_*100):.0f}%" if nnc_ else "-","%Dev":f"{(nv_/nnc_*100):.0f}%" if nnc_ else "-"})
                if pr:
                    dfpr=pd.DataFrame(pr);sc_={"Facturado":"Facturado","Pedidos":"Pedidos","Ads":"Ads","Entregados":"Ent"}[sb]
                    dfpr=dfpr.sort_values(sc_,ascending=False)
                    h="<tr>"+"".join(f"<th>{c}</th>" for c in ["Producto","Pedidos","Facturado","Ent","% Ent","Can","% Can","Trá","% Trá","Dev","% Dev","Ads"])+"</tr>"
                    rb=""
                    for _,r in dfpr.iterrows():
                        rb+=f'<tr><td style="font-weight:500">{r["Producto"]}</td><td>{r["Pedidos"]}</td><td class="mono">{fmt(r["Facturado"],pn)}</td><td style="color:{C["profit"]}">{r["Ent"]}</td><td style="color:{C["profit"]}">{r["%Ent"]}</td><td style="color:{C["loss"]}">{r["Can"]}</td><td style="color:{C["loss"]}">{r["%Can"]}</td><td style="color:{C["blue"]}">{r["Tra"]}</td><td style="color:{C["blue"]}">{r["%Tra"]}</td><td style="color:{C["warn"]}">{r["Dev"]}</td><td style="color:{C["warn"]}">{r["%Dev"]}</td><td style="color:{C["loss"]}" class="mono">{fmt(r["Ads"],pn)}</td></tr>'
        ...
        with ct2:
            if not gc: st.warning("Sin productos.")
                ...
                st.session_state[f"prev_dg_{pn}"]=dg

                # FIX #4: product filter and sort
                pf2=st.selectbox("Filtrar",["Todos"]+prods,key=f"pf_proj_{pn}")
                sb2=st.selectbox("Ordenar",["Utilidad","Ingreso","Órdenes"],key=f"sb_proj_{pn}")
                ixo=k["fact_neto"]/k["n_desp"] if k["n_desp"]>0 else 0
                prows=[]
                for prod in prods:
                    ...
                    pp=dp["PRECIO PROVEEDOR"].mean() if F.get("PRECIO PROVEEDOR") else 0;fp=dp["PRECIO FLETE"].mean() if F.get("PRECIO FLETE") else 0
                    pe=st.session_state[pek].get(prod,float(dg));oe=od*pe/100;odv=od-oe;ue=uds*pe/100;cp=ue*pp;fl=oe*fp+odv*fp*rb_;ap=gxp.get(prod.upper(),0);ip=oe*ixo;ut=ip-cp-fl-ap
                ...
                if not dfp.empty:
                    ...
                    st.markdown(f'<div class="row2 r3"><div class="kcard"><div class="icon w">📦</div><div class="lbl">ÓRDENES</div><div class="val lg white">{dfp["Órdenes"].sum():,}</div></div><div class="kcard green"><div class="icon g">📊</div><div class="lbl">INGRESO PROY.</div><div class="val lg green">{fmt(ti,pn)}</div></div><div class="kcard {"green" if tu>=0 else "red"}"><div class="icon {"g" if tu>=0 else "r"}">📈</div><div class="lbl">UTILIDAD PROY.</div><div class="val lg {"green" if tu>=0 else "red"}">{fmt(tu,pn)}</div></div></div>',unsafe_allow_html=True)
                    ...
                    for _,r in dfp.iterrows():
                        ...
                        bh+=f'<div style="margin-bottom:1.2rem"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="color:{C["text"]};font-weight:600;font-size:.85rem">{r["Producto"]}</span><span style="color:{C["sub"]};font-size:.78rem">{int(r["Órdenes"])} · {int(r["% Entrega"])}%</span></div><div style="display:flex;gap:8px;align-items:center"><div style="flex:1;background:#111827;border-radius:4px;height:22px;position:relative;overflow:hidden"><div style="width:{ipp:.0f}%;height:100%;background:rgba(16,185,129,0.2);border-radius:4px"></div><div style="position:absolute;top:0;left:0;width:{utp:.0f}%;height:100%;background:{C["profit"]};border-radius:4px;opacity:.7"></div></div><span style="font-family:JetBrains Mono;font-size:.82rem;color:{utc};width:130px;text-align:right;font-weight:600">{fmt(r["Utilidad"],pn)}</span></div><div style="display:flex;gap:14px;margin-top:3px;font-size:.7rem;color:{C["sub"]}"><span>Ing:{fmt(r["Ingreso"],pn)}</span><span>Cst:-{fmt(r["Costo"],pn)}</span><span>Flt:-{fmt(r["Fletes"],pn)}</span><span>Ads:-{fmt(r["Ads"],pn)}</span></div></div>'
                    st.markdown(f'<div class="kcard" style="padding:1.5rem">{bh}</div>',unsafe_allow_html=True)
                    with st.expander("📝 Editar %"):
                        ed=st.data_editor(dfp,column_config={"% Entrega":st.column_config.NumberColumn("% ENT",min_value=0,max_value=100,step=1,format="%d")},disabled=[c for c in dfp.columns if c!="% Entrega"],use_container_width=True,hide_index=True,key=f"pt_{pn}")
                        if ed is not None:
                            for _,r in ed.iterrows(): st.session_state[pek][r["Producto"]]=r["% Entrega"]

        # ── OPERACIÓN REAL (FIX #1,#5,#6) ──
        with ct3:
            ...
            at2=f'<span style="font-size:.65rem;color:#475569">({fmt_cop(k.get("g_ads_cop",0))} COP)</span>' if pi["moneda"]!="COP" else ""
            st.markdown(f'<div class="kcard {uc}" style="padding:2rem;margin-bottom:1.5rem"><div class="icon {"g" if ur>=0 else "r"}" style="width:48px;height:48px;font-size:1.3rem">💰</div><div class="lbl">UTILIDAD REAL</div><div class="val xl {uc}">{fmt(ur,pn)}</div><div class="pct">Margen: {mg:.1f}%</div></div>',unsafe_allow_html=True)
            st.markdown(f'<div class="row2 r3"><div class="kcard green"><div class="icon g">✅</div><div class="lbl">INGRESO ENT.</div><div class="val md green">{fmt(ir,pn)}</div><div class="pct">100%</div></div><div class="kcard red"><div class="icon r">📦</div><div class="lbl">COSTO PROD.</div><div class="val md red">-{fmt(cpr,pn)}</div><div class="pct">{pof(cpr,ir)}</div></div><div class="kcard red"><div class="icon r">🚚</div><div class="lbl">FLETES ENT.</div><div class="val md red">-{fmt(fe,pn)}</div><div class="pct">{pof(fe,ir)}</div></div></div><div class="row2 r3"><div class="kcard red"><div class="icon r">🎯</div><div class="lbl">ADS</div><div class="val md red">-{fmt(ga,pn)}</div>{at2}<div class="pct">{pof(ga,ir)}</div></div><div class="kcard red"><div class="icon r">⚠️</div><div class="lbl">FLETES DEV.</div><div class="val md red">-{fmt(fd,pn)}</div><div class="pct">{pof(fd,ir)}</div></div><div class="kcard"><div class="icon b">🚚</div><div class="lbl">FLETES TRÁ.</div><div class="val md white">-{fmt(ft_,pn)}</div><div class="pct">{pof(ft_,ir)}</div></div></div>',unsafe_allow_html=True)

            # FIX #5: Cascade with product filter
            ...
            if cpf!="Todos" and gc:
                dp_c=df_ord[df_ord[gc]==cpf]
                de_c=dp_c[dp_c["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT))] if F.get("ESTATUS") else pd.DataFrame()
                ir_c=de_c["TOTAL DE LA ORDEN"].sum() if not de_c.empty else 0
                cpr_c=de_c["PRECIO PROVEEDOR X CANTIDAD"].sum() if (not de_c.empty and "PRECIO PROVEEDOR X CANTIDAD" in de_c.columns) else 0
                fe_c=de_c["PRECIO FLETE"].sum() if (not de_c.empty and F.get("PRECIO FLETE")) else 0
                ap_c=gxp.get(cpf.upper(),0)
                dd_c=dp_c[dp_c["ESTATUS"].apply(lambda s:ms(s,STATUS_DEV))] if F.get("ESTATUS") else pd.DataFrame()
                fd_c=dd_c["PRECIO FLETE"].sum() if not dd_c.empty else 0
                dt_c=dp_c[dp_c["ESTATUS"].apply(lambda s:ms(s,STATUS_TRA))] if F.get("ESTATUS") else pd.DataFrame()
                ft_c=dt_c["PRECIO FLETE"].sum() if not dt_c.empty else 0
                ir,cpr,fe,ga,fd,ft_=ir_c,cpr_c,fe_c,ap_c,fd_c,ft_c; ur=ir-cpr-fe-ga-fd-ft_
            ...
            if gc and gc in df_ord.columns:
                ...
                for prod in prods:
                    ...
                    ap_p=gxp.get(prod.upper(),0); ut_p=ir_p-cp_p-fe_p-ap_p
                ...
                if pnl:
                    ...
                    h2="<tr>"+"".join(f"<th>{c}</th>" for c in ["Producto","Ingreso","Costo","Fletes","Ads","Utilidad","Margen"])+"</tr>"
                    rb2=""
                    for _,r in dp_.iterrows():
                        utc=C["profit"] if r["Utilidad"]>=0 else C["loss"];mg2=pof(r["Utilidad"],r["Ingreso"])
                        rb2+=f'<tr><td style="font-weight:500">{r["Producto"]}</td><td class="mono">{fmt(r["Ingreso"],pn)}</td><td class="mono" style="color:{C["loss"]}">-{fmt(r["Costo"],pn)}</td><td class="mono" style="color:{C["loss"]}">-{fmt(r["Fletes"],pn)}</td><td class="mono" style="color:{C["loss"]}">-{fmt(r["Ads"],pn)}</td><td class="mono" style="color:{utc}">{fmt(r["Utilidad"],pn)}</td><td style="color:{utc}">{mg2}</td></tr>'
        ...
        with ct4:
            ...
            if cols:
                ...
                with c2: st.markdown(f'<p style="text-align:center;color:#64748B;font-size:.85rem">{st.session_state[opk]+1}/{tp}</p>',unsafe_allow_html=True)
                with c3:
                    if st.button("→",disabled=st.session_state[opk]>=tp-1,key=f"nx_{pn}"): st.session_state[opk]+=1;st.rerun()
                # Download all
                st.download_button("📥 Descargar todas las órdenes",to_excel(df_ord),file_name=f"ordenes_{pn.lower()}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",key=f"dl_all_{pn}")
...
with tabs[-1]:
    ...
    top=sum(cd["kpis"]["n_ord"] for cd in CD.values());tep=sum(cd["kpis"]["n_ent"] for cd in CD.values())
    cpo=tac/top if top>0 else 0;cpe=tac/tep if tep>0 else 0
    st.markdown(f'<div class="row2 r4"><div class="kcard purple"><div class="icon p">📢</div><div class="lbl">TOTAL ADS</div><div class="val lg purple">{fmt_cop(tac)}</div></div><div class="kcard blue"><div class="icon b">📘</div><div class="lbl">FB</div><div class="val lg blue">{fmt_cop(tfc)}</div></div><div class="kcard"><div class="icon w">🎵</div><div class="lbl">TT</div><div class="val lg white">{fmt_cop(ttc)}</div></div><div class="kcard red"><div class="icon r">🎯</div><div class="lbl">CPA</div><div class="val lg red">{fmt_cop(cpo)}</div><div class="sub">Ent: {fmt_cop(cpe)}</div></div></div>',unsafe_allow_html=True)
    if gxp:
        pa=pd.DataFrame([{"Producto":k,"Gasto":v} for k,v in sorted(gxp.items(),key=lambda x:-x[1])])
        fp=px.bar(pa,x="Producto",y="Gasto",color_discrete_sequence=[C["purple"]]);fp.update_layout(**pl(title="GASTO POR PRODUCTO"));fp.update_traces(hovertemplate="%{y:,.0f}<extra></extra>")
        st.plotly_chart(fp,use_container_width=True,key="ap")
    else: st.info("Mapea campañas → productos para ver gasto por producto.")

st.divider()
st.markdown(f'<div style="text-align:center;color:#475569;font-size:.75rem">Profit Dashboard v5.0 · {sum(1 for p in PAISES if st.session_state.get(f"_b_{p}"))} países · {f_ini.strftime("%d/%m")} – {f_fin.strftime("%d/%m/%Y")}</div>',unsafe_allow_html=True)
