...
CRITICAL FIX: Each row = product line (no groupby ID for money)
Projection uses per-delivered-order averages (exact match)
"""
...
.cas-lbl{width:170px;font-size:.82rem;color:#94A3B8;flex-shrink:0}.cas-bar-wrap{flex:1;height:26px;position:relative;margin:0 16px}.cas-bar{height:100%;border-radius:4px;min-width:4px}
.cas-amt{width:140px;text-align:right;font-family:'JetBrains Mono',monospace;font-weight:600;font-size:.88rem;flex-shrink:0}.cas-pct{width:60px;text-align:right;font-family:'JetBrains Mono',monospace;font-size:.72rem;color:#64748B;flex-shrink:0}
...
def get_trm():
    ...
    try:
        x=requests.get("https://open.er-api.com/v6/latest/USD",timeout=10)
        if x.ok: d=x.json();r["COP_USD"]=d["rates"].get("COP",4200);r["COP_GTQ"]=r["COP_USD"]/d["rates"].get("GTQ",7.75)
    except: pass
    return r
def cop_to(v,t,trm):
    if t=="COP": return v
...
def detect_country(df):
    ...
    if "CIUDAD DESTINO" in df.columns:
        c=set(df["CIUDAD DESTINO"].dropna().str.upper().str.strip().unique());sc={"Colombia":len(c&co),"Ecuador":len(c&ec),"Guatemala":len(c&gt)};b=max(sc,key=sc.get)
        if sc[b]>0: return b
    return None
def extraer_base(n):
    n=re.sub(r'\s*-\s*',' ',str(n).strip().upper());w=[x for x in n.split() if not re.match(r'^\d+$',x) and x not in ("X","DE","EL","LA","EN","CON","PARA","POR")]
...
def status_pill(s):
    ...
    if "CANCELADO" in s: return '<span class="pill p-can">⊘ Can</span>'
    if "DEVOLUCION" in s or "DEVOLUCIÓN" in s: return '<span class="pill p-dev">↩ Dev</span>'
    if ms(s,STATUS_TRA): return '<span class="pill p-tra">🚚 Trá</span>'
    if "NOVEDAD" in s: return '<span class="pill p-nov">⚠ Nov</span>'
    return '<span class="pill p-pen">⏳ Pen</span>'

# ═══ CRITICAL FIX: NO groupby(ID) for money ═══
# Each row = 1 product line with ALREADY-CORRECT split values
# Order count = nunique("ID")
# Money = direct sum of all rows (per product filter)
def get_F(df):
...
def calc_kpis(df,F,gfb,gtt):
    """Calculate KPIs from RAW rows (no groupby)."""
    no=df["ID"].nunique() if F.get("ID") else len(df)
    fb=df["TOTAL DE LA ORDEN"].sum() if F.get("TOTAL DE LA ORDEN") else 0
    dnc=df[~df["ESTATUS"].apply(lambda s:ms(s,STATUS_CAN))] if F.get("ESTATUS") else df
    ...
    # Status counts by unique IDs
    ne=nc=nt=nv=nn=0
    if F.get("ESTATUS") and F.get("ID"):
        for s in df["ESTATUS"].unique():
            ids=df[df["ESTATUS"]==s]["ID"].nunique()
            if ms(s,STATUS_ENT): ne+=ids
            elif ms(s,STATUS_CAN): nc+=ids
            elif ms(s,STATUS_TRA): nt+=ids
            elif ms(s,STATUS_DEV): nv+=ids
            elif ms(s,STATUS_NOV): nn+=ids
    nother=max(0,no-ne-nc-nt-nv-nn);nnc=no-nc
    pcan=(nc/no*100) if no>0 else 0;pent=(ne/nnc*100) if nnc>0 else 0
    # Financials from rows (NOT grouped)
    de=df[df["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT))] if F.get("ESTATUS") else pd.DataFrame()
    ...
    dd=df[df["ESTATUS"].apply(lambda s:ms(s,STATUS_DEV))] if F.get("ESTATUS") else pd.DataFrame()
    fld=dd[["PRECIO FLETE","COSTO DEVOLUCION FLETE"]].max(axis=1).sum() if (not dd.empty and F.get("PRECIO FLETE") and F.get("COSTO DEVOLUCION FLETE")) else (dd["PRECIO FLETE"].sum() if not dd.empty and F.get("PRECIO FLETE") else 0)
    dt=df[df["ESTATUS"].apply(lambda s:ms(s,STATUS_TRA))] if F.get("ESTATUS") else pd.DataFrame()
...
def product_pnl(df,gc,F,lgxp):
    """Full P&L per product from raw rows."""
    rows=[]
    for prod in df[gc].dropna().unique():
        ...
        fd=dd["PRECIO FLETE"].sum() if not dd.empty else 0
        dt=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_TRA))] if F.get("ESTATUS") else pd.DataFrame()
        ft=dt["PRECIO FLETE"].sum() if not dt.empty else 0
        ap=lgxp.get(prod.upper(),0);ut=ir-cp-fe-fd-ft-ap
        rows.append(dict(Producto=prod,Ingreso=ir,Costo=cp,FlEnt=fe,FlDev=fd,FlTra=ft,Ads=ap,Utilidad=ut))
    return pd.DataFrame(rows) if rows else pd.DataFrame()

# FIX: Projection uses per-DELIVERED-ORDER averages (exact match validated)
def calc_projection(df,gc,F,pe_dict,rb,lgxp):
    """Projection using actual per-delivered-order averages. Matches real data exactly at real %."""
    rows=[]
    for prod in df[gc].dropna().unique():
        dp=df[df[gc]==prod]
        n_can=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_CAN))]["ID"].nunique() if F.get("ESTATUS") and F.get("ID") else 0
        n_ord=dp["ID"].nunique() if F.get("ID") else len(dp)
        n_nc=n_ord-n_can
        de=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT))] if F.get("ESTATUS") else pd.DataFrame()
        ...
        # Per-delivered-order averages from ACTUAL data
        aov=ir_real/n_ent_real if n_ent_real else 0
        cpoe=cp_real/n_ent_real if n_ent_real else 0
        floe=fl_real/n_ent_real if n_ent_real else 0
        # Non-delivered flete average (for dev/tra cost estimation)
        dnd=dp[~dp["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT)|ms(s,STATUS_CAN))] if F.get("ESTATUS") else pd.DataFrame()
        ...
        rows.append({"Producto":prod,"Órdenes":int(n_nc),"% Ent":pe,"Ingreso":round(ip),"Costo":round(cp_),"FlEnt":round(fl_ent),"FlResto":round(fl_rest),"Ads":round(ap),"Utilidad":round(ut)})
    return pd.DataFrame(rows) if rows else pd.DataFrame()
...
def tt_spend(tok,aid,i,f):
    ...
    return 0.0
def ai_map_testeo(ci,pbc,key):
    if not key: return {}
    prompt=f"Empareja campañas FB con productos Dropi.\nFormato: [PAÍS]-[PRODUCTO/TESTEO]-[FECHA]\nCO=Colombia,EC=Ecuador,GT=Guatemala.\nSi tiene TESTEO, asignar a 'TESTEO CO/EC/GT'.\n\nCampañas:{json.dumps(ci,ensure_ascii=False)}\nProductos:{json.dumps(pbc,ensure_ascii=False)}\nJSON:{'{'}\"campaña\":\"producto\"{'}'}"
    try:
        ...
        txt=r.json()["choices"][0]["message"]["content"].strip();txt=re.sub(r'^```json\s*','',txt);txt=re.sub(r'```$','',txt);return json.loads(txt)
    except: return {}
...
with st.sidebar:
    ...
    if usar_api:
        ...
        if accts:
            ...
            if st.checkbox("✅ Todas",key="sa"): fb_cids=[opts[k] for k in ok]
            else: fb_cids=[opts[l] for l in st.multiselect("Cuentas",ok,key="fm")]
        else:
...
presets={"Hoy":(hoy,hoy),"Ayer":(hoy-timedelta(days=1),hoy-timedelta(days=1)),"Últimos 7d":(hoy-timedelta(days=7),hoy),"Últimos 30d":(hoy-timedelta(days=30),hoy),"Este mes":(hoy.replace(day=1),hoy),"Mes anterior":((hoy.replace(day=1)-timedelta(days=1)).replace(day=1),hoy.replace(day=1)-timedelta(days=1))}
pc1,pc2,pc3,pc4=st.columns([2,1,1,2])
with pc1: preset=st.selectbox("📅",list(presets.keys())+["Personalizado"],index=3,key="ps")
if preset=="Personalizado":
    with pc2: f_ini=st.date_input("Desde",value=hoy-timedelta(days=30),key="di")
    with pc3: f_fin=st.date_input("Hasta",value=hoy,key="df")
else: f_ini,f_fin=presets[preset]
with pc4: st.markdown(f'<div style="padding-top:1.6rem"><span style="color:{C["sub"]};font-size:.8rem">TRM: <b style="color:{C["text"]}">1 USD={trm_usd:,.0f}</b> · <b style="color:{C["text"]}">1 GTQ≈{trm_gtq:,.0f}</b> COP</span></div>',unsafe_allow_html=True)

any_data=any(st.session_state.get(f"_b_{pn}") for pn in PAISES)
if not any_data:
    st.divider()
    files=st.file_uploader("📁 Sube archivos",type=["csv","xlsx","xls"],accept_multiple_files=True,key="um")
    if files:
        for f in files:
    ...
    for pn,pi in PAISES.items():
        f=st.file_uploader(f"{pi['flag']} {pn}",type=["csv","xlsx","xls"],key=f"um_{pn}")
...
with st.sidebar:
    ...
    for pn,pi in PAISES.items():
        if st.session_state.get(f"_b_{pn}"):
            ...
            with c2:
                if st.button("🗑",key=f"d_{pn}"): st.session_state[f"_b_{pn}"]=None;st.session_state[f"_n_{pn}"]=None;st.rerun()
        f=st.file_uploader(f"{pi['flag']}",type=["csv","xlsx","xls"],key=f"us_{pn}",label_visibility="collapsed")
        if f: st.session_state[f"_b_{pn}"]=f.getvalue();st.session_state[f"_n_{pn}"]=f.name;st.rerun()
...
for pn in PAISES:
    ...
    dft=filtrar(cargar(st.session_state[f"_b_{pn}"],st.session_state[f"_n_{pn}"]),f_ini,f_fin)
    if not dft.empty: crl[pn]=len(dft);tr_+=len(dft)
for pn in PAISES:
    if not st.session_state.get(f"_b_{pn}"): continue
...
# Product grouping + Campaign mapping (same as v5.2)
    ...
    with st.expander("🔗 Campañas → Productos",expanded=False):
        ...
        with a1:
            if st.button("🪄 IA",key="ai"):
                if oai_key:
                    asgn=set();[asgn.update(v) for v in st.session_state["cd"].values()]
                    ...
                    if res:
                        for camp,prod in res.items(): st.session_state["cd"].setdefault(prod,[]);st.session_state["cd"][prod].append(camp) if camp not in st.session_state["cd"].get(prod,[]) else None
                        st.rerun()
        with a2:
            ta=sum(len(v) for v in st.session_state["cd"].values());st.caption(f"{len(cn)} camps · {ta} asignadas · {len(cn)-ta} sin")
        search=st.text_input("🔍",key="cs",placeholder="Filtrar...")
        for pn2,prod in all_prods:
            curr=st.session_state["cd"].get(prod,[])
            other=set();[other.update(v) for k,v in st.session_state["cd"].items() if k!=prod]
            avail=[c for c in cn if c not in other]
            if search: avail=[c for c in avail if search.upper() in c.upper()]
            opts=[f"{c} (${camp_spend.get(c,0):,.0f})" for c in avail];omap={f"{c} (${camp_spend.get(c,0):,.0f})":c for c in avail}
        ...
        st.divider()
        asgn2=set();[asgn2.update(v) for v in st.session_state["cd"].values()]
        un=[c for c in cn if c not in asgn2]
        if un: st.markdown(f"⚠️ **{len(un)} sin asignar:**");[st.caption(f"• {c} — ${camp_spend.get(c,0):,.0f}") for c in un[:20]]
        if st.button("💾 Guardar",key="sc",type="primary"): cm_saved=dict(st.session_state["cd"]);sj(CAMP_MAPPING_FILE,cm_saved);st.success("✅");st.rerun()
...
with tabs[0]:
    ...
    ad=[]
    for pn,cd in CD.items():
        if "FECHA" in cd["df"].columns:
            d=cd["df"].groupby(cd["df"]["FECHA"].dt.date).agg(fac=("TOTAL DE LA ORDEN","sum"),ords=("ID","nunique")).reset_index();d.columns=["Fecha","Fac","Ords"];d["Fac"]=d["Fac"].apply(lambda v:to_cop(v,PAISES[pn]["moneda"],trm));ad.append(d)
    if ad:
    ...
    st.divider()
    for pn,cd in CD.items():
        k=cd["kpis"];pi=PAISES[pn];uc="green" if k["u_real"]>=0 else "red";cn_=f'<span style="font-size:.65rem;color:#475569">({fmt_cop(k.get("g_ads_cop",0))} COP)</span>' if pi["moneda"]!="COP" else ""
    ...
    for pn,cd in CD.items():
        ...
        with dc3: dl_log(cd["df"],cd["F"],"Tránsito",STATUS_TRA,f"dt_{pn}")
...
with tabs[1]:
    ...
    for i,(pn,cd) in enumerate(CD.items()):
        with cc[i]: g_pe[pn]=st.slider(f"%Ent {PAISES[pn]['flag']}",50,100,80,key=f"gpe_{pn}");g_rb[pn]=st.number_input(f"Colchón {PAISES[pn]['flag']}",value=1.4,min_value=1.0,max_value=3.0,step=0.05,key=f"grb_{pn}")
    all_proj=[];proj_det={}
    for pn,cd in CD.items():
        gc_=gcol(cd)
        if not gc_: continue
        ...
        if not dfp.empty:
            ...
            dfp["I_COP"]=dfp["Ingreso"].apply(lambda v:to_cop(v,PAISES[pn]["moneda"],trm));dfp["U_COP"]=dfp["Utilidad"].apply(lambda v:to_cop(v,PAISES[pn]["moneda"],trm))
    ...
    if all_proj:
        ...
        st.divider();st.markdown('<p class="section-hdr">Detalle</p>',unsafe_allow_html=True)
    ...
    with tabs[idx+2]:
        ...
        with ct1:
            ...
            if gc_ and F.get("FECHA"):
                ...
                with g2:
                    if F.get("ESTATUS"):
                        edf=dch["ESTATUS"].value_counts().reset_index();edf.columns=["E","N"];cm_={s:(C["profit"] if "ENTREGADO" in s else C["loss"] if "CANCELADO" in s else C["blue"] if ms(s,STATUS_TRA) else C["muted"]) for s in edf["E"]}
            ...
            with dc3: dl_log(df_f,F,"Tránsito",STATUS_TRA,f"dt2_{pn}")
            ...
            if gc_:
                ...
                for prod in prods:
                    ...
                    np_=dp["ID"].nunique();de_=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT))];ne_=de_["ID"].nunique();nc_=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_CAN))]["ID"].nunique()
                    nt_=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_TRA))]["ID"].nunique();nv_=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_DEV))]["ID"].nunique()
                    fp_=de_["TOTAL DE LA ORDEN"].sum();nnc_=np_-nc_;ap_=lgxp.get(prod.upper(),0)
        ...
        with ct3:
            ...
            if cpf!="Todos" and gc_:
                dp=df_f[df_f[gc_]==cpf];de=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_ENT))];c_ir=de["TOTAL DE LA ORDEN"].sum() if not de.empty else 0;c_cpr=de["PRECIO PROVEEDOR X CANTIDAD"].sum() if not de.empty else 0;c_fe=de["PRECIO FLETE"].sum() if not de.empty else 0;c_ga=lgxp.get(cpf.upper(),0)
                dd=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_DEV))];c_fd=dd["PRECIO FLETE"].sum() if not dd.empty else 0;dt=dp[dp["ESTATUS"].apply(lambda s:ms(s,STATUS_TRA))];c_ft=dt["PRECIO FLETE"].sum() if not dt.empty else 0
            ...
            for lb,vl,pos in items:
                bp=min(vl/mx_c*100,100);bc=C["profit"] if pos else C["loss"];ch+=f'<div class="cas-row"><div class="cas-lbl">{lb}</div><div class="cas-bar-wrap"><div class="cas-bar" style="width:{bp:.0f}%;background:{bc}"></div></div><div class="cas-amt" style="color:{bc}">{"" if pos else "-"}{fmt(vl,pn)}</div><div class="cas-pct">{pof(vl,c_ir)}</div></div>'
        ...
        with ct4:
            cols=[c for c in ["ID","FECHA","PRODUCTO","CANTIDAD","TOTAL DE LA ORDEN","PRECIO FLETE","CIUDAD DESTINO","ESTATUS"] if c in df_f.columns]
            if cols:
                dfo=df_f[cols].copy()
...
st.divider()
st.markdown(f'<div style="text-align:center;color:#475569;font-size:.75rem">v5.3 · {sum(1 for p in PAISES if st.session_state.get(f"_b_{p}"))} países · {f_ini.strftime("%d/%m")}–{f_fin.strftime("%d/%m/%Y")}</div>',unsafe_allow_html=True)
