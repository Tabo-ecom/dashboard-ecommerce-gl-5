...
def process_data(df, start, end, country_name, g_fb_total, g_tt_total, campaign_map):
    ...
    agg_rules = {k:v for k,v in {"TOTAL DE LA ORDEN":"first","ESTATUS":"first","PRECIO FLETE":"first","COSTO DEVOLUCION FLETE":"first","GANANCIA":"sum","FECHA":"first","PRODUCTO":"first","CANTIDAD":"sum","PRECIO PROVEEDOR":"first"}.items() if k in df.columns}
...
if not df_ads_raw.empty and uploaded_data:
    with st.expander("🔗 Mapeo de Productos (IA + Manual)", expanded=False):
        ...
        with c_m1: sel_camp = st.selectbox("Campaña FB", [""] + unmapped_camps)
        with c_m2: sel_prod = st.selectbox("Producto Dropi", [""] + sorted(list(all_prods)))
        with c_m3:
